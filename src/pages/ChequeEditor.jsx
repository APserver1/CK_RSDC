...
export default function ChequeEditor({ Layout }) {
  ...
  const [rateTables, setRateTables] = useState(LOCAL_VIATICO_RATE_TABLES)
  const [rateRows, setRateRows] = useState(LOCAL_VIATICO_RATE_ROWS)
  const [rateTableCode, setRateTableCode] = useState('legacy')
  ...
  useEffect(() => {
    ...
    getCheque(editId).then(({ data, error }) => {
      ...
      setShowBaseSalary(Boolean(stored.showBaseSalary))
      setRateTableCode(stored.rateTableCode || 'legacy')
  ...
  useEffect(() => {
    Promise.all([
      fetchAndCacheLookup('viatico-rate-tables', async () => {
        if (!supabase) return null
        const { data, error } = await supabase.from('ck_viatico_rate_tables').select('codigo, nombre, vigente_desde').order('vigente_desde', { ascending: false, nullsFirst: true })
        if (error) throw error
        return data || []
      }, LOCAL_VIATICO_RATE_TABLES),
      fetchAndCacheLookup('viatico-rate-rows', async () => {
        if (!supabase) return null
        const { data, error } = await supabase.from('ck_viatico_rates').select('tabla_codigo, categoria, zona, valor').order('vigente_desde', { ascending: false })
        if (error) throw error
        return data || []
      }, LOCAL_VIATICO_RATE_ROWS),
    ]).then(([tables, rows]) => {
      setRateTables(tables?.length ? tables : LOCAL_VIATICO_RATE_TABLES)
      setRateRows(rows?.length ? rows : LOCAL_VIATICO_RATE_ROWS)
    })
  }, [])
  ...
  const rateTableMap = rateRows.reduce((tables, row) => {
    const table = tables[row.tabla_codigo] || {}
    table[row.categoria] = { ...(table[row.categoria] || {}), [String(row.zona)]: Number(row.valor) }
    tables[row.tabla_codigo] = table
    return tables
  }, {})
  const selectedTableRates = rateTableMap[rateTableCode] || {}
  const zoneRates = Object.keys(savedZoneRates).length ? savedZoneRates : (selectedTableRates[category] || {})
  const editorData = { setup, beneficiary, identity, chequeDescription, position, salary, level, category, showBaseSalary, rateTableCode, department, city, residence, travelDays, travelStart, travelEnd, vehicleType, plate, driver, expenseRows, itineraryRows, viaticoRates: zoneRates }
  ...
  return <Layout><div className="editor-page">
    <div className="editor-toolbar"><button className="back-button" onClick={() => navigate(-1)}>← Regresar</button>{started && <div><span className="editor-kicker">CHEQUE {setup.number}</span><strong>{setup.type === 'Viatico' ? 'Viático' : 'Compra'}</strong></div>}{started && <div className="save-area"><button className="print-button" type="button" onClick={() => window.print()}>Imprimir</button>{setup.type === 'Viatico' && <label className="salary-toggle"><input type="checkbox" checked={showBaseSalary} onChange={event => setShowBaseSalary(event.target.checked)} /><span>B/SUELDO</span></label>}<button className="save-button" type="button" onClick={() => saveCheque()} disabled={saveState === 'saving'}>{saveState === 'saving' ? 'Guardando…' : 'Guardar'}</button>{saveState === 'saved' && <small>Guardado {lastSavedAt?.toLocaleTimeString('es-HN')}</small>}{saveState === 'offline' && <small className="save-offline">Guardado local · pendiente de sincronizar</small>}{saveState === 'error' && <small className="save-error">No se pudo guardar</small>}</div>}</div>
